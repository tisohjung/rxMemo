...
class MemoListViewModel: CommonViewModel {
        ...
        return CocoaAction { _ in
            return self.storage.createMemo(content: "")
                .flatMap { memo -> Observable<Void> in
                    ...
                    return self.sceneCoordinator.transition(to: composeScene, using: TransitionType.modal, animated: true).asObservable().map { _ in }
                }
        }
    }
    ...
    lazy var detailAction: Action<Memo, Void> = {
        return Action { memo in
            ...
            return self.sceneCoordinator.transition(to: detailScene, using: TransitionType.push, animated: true).asObservable().map({ _ in })
        }
    }()
